...
import {
    validateConnectionConfig, validateInsertRecords, validateDetokenizeInput, validateGetByIdInput, validateInitConfig, validateDeleteByIdInput,
} from './utils/validators';

import {
    ContentType,
    IDeleteByIdInput,
    ISkyflowIdRecordWithoutReveal,
    TYPES,
} from './utils/common';
import {
    printLog,
...
class Controller {
    ...
    getById(getByIdInput: IGetByIdInput): Promise<{ records: any[] }> {
        return new Promise((resolve, reject) => {
            try {
                validateInitConfig(this.#client.config)
    ...
    deleteById(deleteByIdInput: IDeleteByIdInput): Promise<{ records: ISkyflowIdRecordWithoutReveal[] }> {
        return new Promise((resolve, reject) => {
            try {
                validateInitConfig(this.#client.config)
                printLog(logs.infoLogs.VALIDATE_DELETE_BY_ID_INPUT, MessageType.LOG);
                validateDeleteByIdInput(deleteByIdInput);
                printLog(parameterizedString(logs.infoLogs.EMIT_REQUEST,
                    TYPES.DELETE_BY_SKYFLOWID),
                    MessageType.LOG);
                this.getToken().then((res) => {
                    deleteRecordsBySkyflowID(
                        deleteByIdInput.records,
                        this.#client,
                        res
                    ).then(
                        (resolvedResult) => {
                            printLog(logs.infoLogs.DELETE_BY_SKYFLOWID_RESOLVED, MessageType.LOG);
                            resolve(resolvedResult);
                        },
                        (rejectedResult) => {
                            reject(rejectedResult);
                ...
                this.#client
                    .request({
                        ...
                        url:
                            ...
                            }/v1/vaults/${this.#client.config.vaultID}`,
                        headers: {
